...
export class CartController {
  ...
  static async increment(req: AuthRequest, res: Response) {
    const productId = Number(req.params.productId);
    const item = await cartService.incrementItem(req.user!.userId, productId);
    res.json(item);
  }

  static async decrement(req: AuthRequest, res: Response) {
    const productId = Number(req.params.productId);
    const result = await cartService.decrementItem(req.user!.userId, productId);
    res.json(result);
  }
